"""Verify CBS-first fallback also produces zero collisions."""
import sys, os
...
# --- CBS first (will fail) ---
print("\n--- Trying CBS ---")
cbs = CBS(agents=agents, rows=ROWS, cols=COLS, obstacles=OBSTACLES, max_nodes=50000)
solution = cbs.solve()

if solution is None:
    print("CBS failed, resetting agents and falling back to PP...")
    for a in agents:
        a.path = []
        a.fetch_end_t = 0
        a.deliver_end_t = 0
...
for t in range(max_t):
    ...
    for pos, aids in pod_positions.items():
        if len(aids) > 1:
            ...
            print(f"[COLLISION] Tick {t:>3} | Pods {ids_str} at ({pos[0]},{pos[1]})")
            collision_count += 1

if collision_count == 0:
    print("\n[OK] ZERO pod collisions (CBS->PP fallback path)!")
else:
    print(f"\n[FAIL] {collision_count} pod collision(s)!")
